...
<div class="main">
  ...
  <div class="main-right r">
    <div class="nav">您当前的位置：<a href="system/index.htm">管理首页</a> > 其他归档</div>
    <div class="m_m tab">
    	<div class="submenu">
    		<div><a href="#" class="h">其他归档列表</a></div>
    		<div class="clear"></div>
    	</div>
    	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tb_1">
    		<tr>
    			<td colspan="8" >
    				<form action="archive/list_admin.htm?pu.pageNum=1" method="post" id="search">
    		...
    		<c:forEach items="${list}" var="l">
    			<tr align="center">
    				...
    				<td>  <a href="archive/archive.htm?id=${l.id}">查看</a> <!--  |   <a href="#">修改</a>-->   
	    				<c:if test="${user.isAdmin == 'Y' }">
	    					| <a href="javascript:cancel('archive/delete.htm','${l.id}')">作废</a>  
	    				</c:if>
    				</td>
    			</tr>
    		</c:forEach>
    	...
    	<div class="page" align="left">
    		<a href="javascript:to_page('archive/list_admin.htm?pu.pageNum=1','search')">首页</a>
    		<a href="javascript:to_page('archive/list_admin.htm?pu.pageNum=${pu.pageNum-1}','search')">上一页</a>
    		<c:forEach items="${pu.pageList}" var="p">
    			...
    			<c:if test="${p != pu.pageNum}">
    				<a href="javascript:to_page('archive/list_admin.htm?pu.pageNum=${p}','search')">&nbsp; ${p} &nbsp;</a>
    			</c:if>
    		</c:forEach>
    		<a href="javascript:to_page('archive/list_admin.htm?pu.pageNum=${pu.pageNum+1}','search')">下一页</a>
    		<a href="javascript:to_page('archive/list_admin.htm?pu.pageNum=${pu.pageCount}','search')">尾页</a>
    	</div>
    </div>
  </div>
  <div class="clear"></div>
