...
<div class="main">
  ...
  <div class="main-right r">
    <div class="nav">您当前的位置：<a href="system/index.htm">管理首页</a> > 我的流程</div>
    <div class="m_m tab">
    	
    	<div class="submenu">
    		<div><a href="javascript:void(0)" class="h">我的流程列表</a></div>
    		<div class="clear"></div>
    	</div>
    	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tb_1">
    		<tr>
    			<td colspan="7">
    				<form action="userapprove/list_all.htm?pu.pageNum=1" method="post" id="search">
    		...
    		<c:forEach items="${userApproves}" var="ua">
    			<tr align="center">
    				...
    				<td><span>
    					<c:if test="${ua.status == 'W'}">
    						等待审批
	    				</c:if>
    					<c:if test="${ua.status == 'N'}">
    				...
    				<a href="userapprove/approvelog.htm?id=${ua.id}">查看</a>
    				 ...
    				 <c:if test="${user.isAdmin == 'Y' }">
    				 	| <a href="javascript:cancel('userapprove/approve_delete.htm','${ua.id}')">作废</a>
    				 </c:if>
    	...
    	<div class="page" align="left">
    		<a href="javascript:to_page('userapprove/list_all.htm?pu.pageNum=1','search')">首页</a>
    		<a href="javascript:to_page('userapprove/list_all.htm?pu.pageNum=${pu.pageNum-1}','search')">上一页</a>
    		<c:forEach items="${pu.pageList}" var="p">
    			...
    			<c:if test="${p != pu.pageNum}">
    				<a href="javascript:to_page('userapprove/list_all.htm?pu.pageNum=${p}','search')">&nbsp; ${p} &nbsp;</a>
    			</c:if>
    		</c:forEach>
    		<a href="javascript:to_page('userapprove/list_all.htm?pu.pageNum=${pu.pageNum+1}','search')">下一页</a>
    		<a href="javascript:to_page('userapprove/list_all.htm?pu.pageNum=${pu.pageCount}','search')">尾页</a>
    	</div>
    </div>
  </div>
  <div class="clear"></div>
